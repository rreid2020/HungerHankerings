...
exports.ensureChannelDefaultCurrency = ensureChannelDefaultCurrency;
/**
...
function mergeSimpleArrayCsv(raw, targetUpper) {
    const parts = (raw ?? "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean)
...
function normalizeCurrencyCsv(raw) {
    return (raw ?? "")
        .split(",")
...
async function resolveChannelTable(client) {
  ...
  `);
    const row = r.rows[0];
    if (!row?.def_col || !row.avail_col || !row.code_col)
        return null;
    return {
        schema: row.schema,
        table: row.tbl,
...
async function ensureChannelDefaultCurrency() {
    ...
    try {
        ...
        const sel = await client.query(`SELECT ${qCode} AS code, ${qDef} AS def, ${qAvail} AS avail FROM ${fullTable}`);
        const list = sel.rows;
        for (const row of list) {
            ...
            if (defaultNeedsWrite) {
                if (!explicitEnv && cur !== "USD") {
                    console.warn(`[ensure-channel-currency] Channel "${row.code}" uses ${cur}, not ${target}. ` +
            ...
            if (!defaultNeedsWrite && !availNeedsWrite)
                continue;
            await client.query(`UPDATE ${fullTable} SET ${qDef} = $1, ${qAvail} = $2 WHERE ${qCode} = $3`, [target, mergedAvail, row.code]);
            console.info(`[ensure-channel-currency] Channel "${row.code}": defaultCurrency ${cur || "(empty)"} → ${target}; availableCurrencyCodes → ${mergedAvail}`);
        }
    }
    finally {
        await client.end();
