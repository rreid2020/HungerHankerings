...
| **Clerk handshake → HTTP 500** | `@clerk/nextjs` **`runHandlerWithRequestState`** throws **`Clerk: handshake status without redirect`** when **`authenticateRequest`** returns **`Handshake`** but **no `Location`** header — common if **`signInUrl` / app URL** are wrong behind a proxy or when the browser cannot complete cross-domain cookie flows to **`clerk.*`**. | Set absolute **`signInUrl`** (`NEXT_PUBLIC_CLERK_SIGN_IN_URL` or derived `https://ops…/ops/sign-in`). Prefer the Clerk FAPI CNAME (`clerk.ops…`, encoded in the publishable key). Only enable **`CLERK_FRONTEND_API_PROXY=1`** if you also configure the Dashboard proxy URL + **`NEXT_PUBLIC_CLERK_PROXY_URL`** — otherwise handshake returns **`host_invalid`**. |
| **Clerk `host_invalid` on `/_clerk` or `/__clerk`** | Frontend API proxy is enabled in middleware but Clerk Dashboard has no matching **proxy URL**, or the browser calls the wrong path. Message blames the publishable key even when the key is correct. | Leave proxy **off** (default). Confirm DNS for **`clerk.ops.hungerhankerings.com`**. Clear site cookies for **`ops…`** and **`clerk…`**, then retry sign-in. |
...
That string is thrown **inside `clerkMiddleware`** before your page runs — it is not a Next.js route bug. Ensure absolute **`signInUrl`** (this repo derives it from **`NEXT_PUBLIC_OPS_HOST`** / **`NEXT_PUBLIC_CLERK_SIGN_IN_URL`**). Use the Clerk FAPI CNAME (`clerk.ops…`) from your publishable key — do **not** enable **`CLERK_FRONTEND_API_PROXY`** unless the Dashboard proxy URL is configured. Clear site cookies for **`ops…`** and **`clerk…`** after changing Clerk URLs.
