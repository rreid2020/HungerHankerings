...
import { effectiveClientScheme, effectiveClientSchemeFromHeaders } from "./public-origin"
...
export function cookieSecureFromRequest(request: NextRequest): boolean {
  const forwarded = effectiveClientScheme(request)
...
export async function cookieSecureFromHeaders(): Promise<boolean> {
  try {
    ...
    const proto = effectiveClientSchemeFromHeaders((name) => h.get(name))
